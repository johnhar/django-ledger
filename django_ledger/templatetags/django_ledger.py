...
def balance_sheet_statement(context, io_model, to_date=None):
    ...
    io_digest = io_model.digest(
        ...
        by_fund=context.get('by_fund', None),
        to_date=to_date,
        signs=True,
        process_groups=True,
...
def cash_flow_statement(context, io_model):
    ...
    io_digest = io_model.digest(
        ...
        entity_slug=entity_slug,
        unit_slug=context.get('unit_slug', None),
        by_unit=context.get('by_unit', False),
        fund_slug=context.get('fund_slug', None),
        by_fund=context.get('by_fund', False),
        from_date=from_date,
        to_date=to_date,
        process_groups=True)
...
def period_navigation(context, base_url: str):
    ...
    if context['view'].kwargs.get('fund_slug'):
        kwargs['fund_slug'] = context['view'].kwargs.get('fund_slug')
    ...
    if 'fund_slug' in kwargs:
        KWARGS_CURRENT_MONTH['fund_slug'] = kwargs['fund_slug']
